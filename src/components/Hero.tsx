"use client";

import { useEffect, useRef } from "react";

export default function Hero() {
  const headingRef = useRef<HTMLHeadingElement>(null);
  const subRef = useRef<HTMLParagraphElement>(null);
  const badgeRef = useRef<HTMLDivElement>(null);
  const ctaRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const els = [headingRef, subRef, badgeRef, ctaRef];
    els.forEach((ref, i) => {
      if (ref.current) {
        ref.current.style.animationDelay = `${300 + i * 200}ms`;
        ref.current.classList.add("animate-fade-in-up");
      }
    });
  }, []);

  return (
    <section className="relative min-h-screen flex items-end justify-center overflow-hidden pb-16 sm:pb-20 md:pb-24">
      {/* Background image */}
      <div className="absolute inset-0">
        <div
          className="absolute inset-0 bg-cover bg-no-repeat"
          style={{
            backgroundImage: `url('/images/hero.jpg')`,
            backgroundPosition: "center 40%",
          }}
        />
        {/* Gradient: light top (faces), dark bottom (text) */}
        <div className="absolute inset-0 bg-gradient-to-b from-black/30 via-transparent via-40% to-black/80" />
      </div>

      {/* Content — always at bottom */}
      <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
        {/* Award badge */}
        <div ref={badgeRef} className="will-animate mb-4 sm:mb-5 inline-block">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 sm:px-5 sm:py-2 rounded-full border border-gold/40 bg-black/20 backdrop-blur-sm text-gold-light text-xs sm:text-sm tracking-wider uppercase">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="sm:w-4 sm:h-4">
              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
            </svg>
            Vítěz Gourmet Brno 2025
          </span>
        </div>

        <h1
          ref={headingRef}
          className="will-animate font-[family-name:var(--font-heading)] text-5xl sm:text-6xl md:text-7xl text-white leading-[0.95] mb-4 drop-shadow-lg"
        >
          Mlsná{" "}
          <span className="italic text-gold-light">holka</span>
        </h1>

        <p
          ref={subRef}
          className="will-animate text-white/80 text-base sm:text-lg md:text-xl max-w-xl mx-auto mb-6 sm:mb-8 leading-relaxed"
        >
          Prémiové dorty na zakázku, makronky &amp; mini donuty.
          <br />
          Pečeme s láskou v Brně od roku 2017.
        </p>

        <div ref={ctaRef} className="will-animate flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
          <a
            href="#dorty"
            className="w-full sm:w-auto px-8 py-4 bg-plum text-white text-sm uppercase tracking-[0.15em] rounded-full hover:bg-plum-light transition-all duration-300 hover:shadow-lg hover:shadow-plum/25"
          >
            Prohlédnout dorty
          </a>
          <a
            href="#objednavka"
            className="w-full sm:w-auto px-8 py-4 border border-white/30 text-white text-sm uppercase tracking-[0.15em] rounded-full hover:bg-white/10 transition-all duration-300 backdrop-blur-sm"
          >
            Objednat na míru
          </a>
        </div>
      </div>

      {/* Scroll indicator — desktop only */}
      <div className="hidden md:flex absolute bottom-4 left-1/2 -translate-x-1/2 flex-col items-center gap-2 text-white/40">
        <span className="text-[10px] uppercase tracking-[0.2em]">Scroll</span>
        <div className="w-px h-6 bg-gradient-to-b from-white/40 to-transparent" />
      </div>
    </section>
  );
}
